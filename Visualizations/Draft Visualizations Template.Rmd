...
```{r}
#Create Labels for Tech Companies
techlabel <- c(
`0` = "Not Primarily a Tech Company",
`1` = "Primarily a Tech Company")
#Create Visualization
t1 <- ggplot(health, aes(x = health$Is.your.anonymity.protected.if.you.choose.to.take.advantage.of.mental.health.or.substance.abuse.treatment.resources.provided.by.your.employer.))+geom_bar()+facet_wrap(health$Is.your.employer.primarily.a.tech.company.organization. ~ ., labeller = as_labeller(techlabel)) + ggtitle("Anonymity Protections For Tech vs. Non-Tech Companies") + xlab("Is your anonymity protected?") + ylab("Count") 
t1
```
...
```{r}
#Modify Labels
pd2 <- factor(health$If.a.mental.health.issue.prompted.you.to.request.a.medical.leave.from.work..asking.for.that.leave.would.be.,
levels = c("Very easy", "Somewhat easy", "Neither easy nor difficult","Somewhat difficult", "Very difficult", "I don't know"),
labels = c("V Easy", "S Easy", "Neutral", "SW Hard", "V Hard", "Don't Know"))
```

```{r}
#Crosstab table
ELDbG <- table(pd2, health$gender)
ELDbG
prop.table(ELDbG, 2)
```
...
```{r}
#Create Visualization
t2 <- ggplot(health, aes(x = pd2))+geom_bar()+facet_grid(health$gender ~ ., scales="free_y") + ggtitle("Perceived Difficulty of Asking for Leave by Gender") + xlab("Perceived Difficulty Level") + ylab("Count") 
t2
```
...
```{r}
#Crosstab table
ELDbA <- table(pd2, health$AgeGroup)
ELDbA
#Proportions table
prop.table(ELDbA)
```
...
```{r}
#Create Visualization
t3 <- ggplot(health, aes(x = pd2))+geom_bar()+facet_grid(health$AgeGroup ~ ., scales="free_y") + ggtitle("Perceived Difficulty of Asking for Leave by Age Group") + xlab("Perceived Difficulty Level") + ylab("Count") 
t3
```
...
```{r}
#Crosstab table
ELDbCS <- table(pd2, health$CompanySize)
ELDbCS
prop.table(ELDbCS, 2)
```
...
```{r}
#Modify Labels
pd3 <- factor(health$If.a.mental.health.issue.prompted.you.to.request.a.medical.leave.from.work..asking.for.that.leave.would.be.,
levels = c("Very easy", "Somewhat easy", "Neither easy nor difficult","Somewhat difficult", "Very difficult", "I don't know"),
labels = c("E", "SE", "N", "SH", "H", "D"))
#Create Visualization
t4 <- ggplot(health, aes(x = pd3))+geom_bar()+facet_grid(~ health$CompanySize) + ggtitle("Perceived Difficulty of Asking for Leave by Company Size") + xlab("Perceived Difficulty Level") + ylab("Count")
t4
```
...
```{r}
t5 <- ggplot(health, aes(x = pd3))+geom_bar()+facet_grid(~ health$mhb2, scales="free_y") + ggtitle("Perceived Difficulty of Asking for Leave by MH Benefit Provision") + xlab("Perceived Difficulty Level") + ylab("Count")
t5
```
NOTE:  E = Very Easy, S = Somewhat Easy, N = Neutral (or Neither Easy nor Difficult), SH = Somewhat Hard, H = Very Hard, D = Don't Know.  
...
```{r}
ELDbEDM <- table(pd2, health$Has.your.employer.ever.formally.discussed.mental.health..for.example..as.part.of.a.wellness.campaign.or.other.official.communication..)
ELDbEDM
```
...
```{r}
t6 <- ggplot(health, aes(x = pd2))+geom_bar()+facet_grid( health$Has.your.employer.ever.formally.discussed.mental.health..for.example..as.part.of.a.wellness.campaign.or.other.official.communication.. ~., scales="free_y") + ggtitle("Perceived Difficulty of Asking for Leave by MH Discussion by Employer") + xlab("Perceived Difficulty Level") + ylab("Count")
t6
```
...
```{r}
ELDbEOR <- table(pd2, health$Does.your.employer.offer.resources.to.learn.more.about.mental.health.concerns.and.options.for.seeking.help.)
ELDbEOR
```
...
```{r}
t7 <- ggplot(health, aes(x = pd2))+geom_bar()+facet_grid( health$Does.your.employer.offer.resources.to.learn.more.about.mental.health.concerns.and.options.for.seeking.help. ~., scales="free_y") + ggtitle("Perceived Difficulty of Asking for Leave by Mental Health Resource Provision") + xlab("Perceived Difficulty Level") + ylab("Count")
t7
```
...
```{r}
ELDbPR <- table(pd2, health$Have.you.observed.or.experienced.an.unsupportive.or.badly.handled.response.to.a.mental.health.issue.in.your.current.or.previous.workplace.)
ELDbPR
prop.table(ELDbPR, 2)
```
...
```{r}
t8 <- ggplot(health, aes(x = pd3))+geom_bar()+facet_grid(~ health$epr) + ggtitle("Perceived Difficulty of Asking for Leave by Exp. with Poor MH Response") + xlab("Perceived Difficulty Level") + ylab("Count")
t8
```
NOTE:  E = Very Easy, S = Somewhat Easy, N = Neutral (or Neither Easy nor Difficult), SH = Somewhat Hard, H = Very Hard, D = Don't Know.  
...
```{r}
t10 <- ggplot(health, aes(x = health$Would.you.feel.comfortable.discussing.a.mental.health.disorder.with.your.direct.supervisor.s..))+geom_bar()+facet_grid(health$AgeGroup ~., scales="free_y") + ggtitle("Comfort with Discussing MH with Supervisor by Age") + xlab("Would you feel comfortable discussing a mental health disorder with your supervisor?") + ylab("Count")
t10
```
